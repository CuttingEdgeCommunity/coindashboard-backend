...
public class CoinGeckoResponseParser {
  ...
  ArrayList<CoinMarketDataDto> CurrentParser(Response response, String currency) {
    ...
    for (int i = 0; i < response.getResponseBody().size(); i++) {
      ...
      try {
        timestamp = TimeFormatter.convertStringToTimestamp(
            response.getResponseBody().get(i).get("last_updated").asText(),
            "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'").getTime();
      ...
      pctChange = response.getResponseBody().get(i).get("price_change_percentage_1h_in_currency")
          .floatValue();
      deltas.add(new DeltaDto(IntervalEnum.ONE_HOUR, pctChange,
          current_price * (pctChange / (100 + pctChange))));
      pctChange = response.getResponseBody().get(i).get("price_change_percentage_24h_in_currency")
          .floatValue();
      deltas.add(new DeltaDto(IntervalEnum.ONE_DAY, pctChange,
          current_price * (pctChange / (100 + pctChange))));
      pctChange = response.getResponseBody().get(i).get("price_change_percentage_7d_in_currency")
          .floatValue();
      deltas.add(new DeltaDto(IntervalEnum.SEVEN_DAY, pctChange,
          current_price * (pctChange / (100 + pctChange))));
      pctChange = response.getResponseBody().get(i).get("price_change_percentage_1h_in_currency")
          .floatValue();
      deltas.add(new DeltaDto(IntervalEnum.THIRTY_DAY, pctChange,
          current_price * (pctChange / (100 + pctChange))));
      quoteMaps.put(currency, new QuoteDto(prices, deltas, timestamp));
      coinMarketDataDtos.add(
          new CoinMarketDataDto(response.getResponseBody().get(i).get("name").asText(),
  ...
  CoinMarketDataDto HistoricalParser(Response response, String name, String vs_currency) {
    ...
    for (int i = 0; i < response.getResponseBody().get("prices").size(); i++) {
      prices.add(new PriceDto(response.getResponseBody().get("prices").get(i).get(1).floatValue(),
          response.getResponseBody().get("prices").get(i).get(0).asLong()));
    }
    quoteMaps.put(vs_currency, new QuoteDto(prices, deltas, response.getResponseBody().get("prices")
        .get(response.getResponseBody().get("prices").size() - 1).get(0).asLong()));
